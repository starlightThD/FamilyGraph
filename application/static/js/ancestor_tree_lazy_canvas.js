(() => {
  const mountEl = document.getElementById('ancestor-tree-vue-app');
  ...
  const normalizeNode = (rawNode) => ({
    ...
    has_parents: Boolean(rawNode.has_parents),
  ...
  const canExpandNode = (node) => {
    ...
    return Boolean(node.has_parents);
  };
  ...
  const computeLayout = (rootNode) => {
    ...
    const measure = (node) => {
      const parents = getVisibleParents(node);
      if (!parents.length) {
    ...
    const place = (node, depth, top) => {
      ...
      const parentTotalHeight = parents.reduce((sum, parent, index) => {
        return sum + parent.__subtreeHeight + (index > 0 ? SIBLING_GAP_Y : 0);
      }, 0);

      let cursor = top + (node.__subtreeHeight - parentTotalHeight) / 2;
      parents.forEach((parent) => {
        edges.push({ from: node, to: parent });
        place(parent, depth + 1, cursor);
        cursor += parent.__subtreeHeight + SIBLING_GAP_Y;
      });
    };
  ...
  const app = Vue.createApp({
    ...
    computed: {
      ...
      loadedNodeCount() {
        ...
        const walk = (node) => {
          count += 1;
          if (node.__loaded) {
            node.parents.forEach((parent) => walk(parent));
          }
        };
        walk(this.rootNode);
        return count;
    ...
    methods: {
      ...
      nodeTitle(node) {
        if (!canExpandNode(node)) return 'No more ancestors to expand';
        if (node.__loading) return 'Loading ancestors';
      ...
      async loadRoot() {
        ...
        try {
          ...
          const response = await fetch(`/api/ancestor-preview/root?${params.toString()}`, {
      ...
      async fetchParents(node) {
        ...
        try {
          ...
          const response = await fetch(`/api/ancestor-preview/node/${node.id}/expand?${params.toString()}`, {
            credentials: 'same-origin',
          });
          const payload = await response.json();
          if (!response.ok || !payload.ok) {
            throw new Error(payload.error || 'Failed to load ancestors.');
          }

          node.parents = (payload.parents || []).map((parent) => normalizeNode(parent));
          node.__loaded = true;
          if (!node.parents.length) {
            node.has_parents = false;
          }
        } catch (error) {
          node.__error = error && error.message ? error.message : 'Failed to load ancestors.';
        } finally {
    ...
    template: `
      <div>
        ...
        <div v-else-if="rootNode">
              ...
              <g :transform="cameraTransform">
                ...
                >
                  ...
                  <text x="108" y="43" class="ancestor-card-meta" text-anchor="middle">
                    {{ node.__error ? 'Load failed' : (node.depth === 0 ? 'Selected Member' : ('Ancestor · Depth ' + node.depth)) }}
                  </text>
          ...
          <div class="tree-hint">Tip: click node to expand/collapse ancestors, drag to pan, wheel to zoom.</div>
        </div>
        <div v-else class="result-box">No ancestor tree data available.</div>
      </div>
    `,
  });

  app.mount('#ancestor-tree-vue-app');
})();
